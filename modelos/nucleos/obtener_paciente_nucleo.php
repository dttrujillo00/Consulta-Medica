...
try{
    ...
    $statement = $pdo->prepare('SELECT p.id_pac FROM nucleo n INNER JOIN nucleo_pac np ON n.id_nuc = np.id_nuc INNER JOIN paciente p ON np.id_pac = p.id_pac WHERE n.id_nuc = (?)');
