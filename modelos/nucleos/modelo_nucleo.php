<?php 
	...
	try {
        $statement = $pdo->prepare('INSERT INTO nucleo SET id_nuc=?, no_nuc=?, dir_nuc=?, funcionamiento_nucleo=?, cond_estr_viv=?, indic_hac=?, eq_dom_bas=?, satis_ingreso=?, eval_nuc=? ON DUPLICATE KEY UPDATE no_nuc=?, dir_nuc=?, funcionamiento_nucleo=?, cond_estr_viv=?, indic_hac=?, eq_dom_bas=?, satis_ingreso=?, eval_nuc=?, id_cons = ?');
        $statement->execute(array($id_nuc, $manzana, $direccion, $id_funcFam, $id_condViv, $id_indiceHac, $id_equipBas, $id_satisFam, $id_evalFam, $manzana, $direccion, $id_funcFam, $id_condViv, $id_indiceHac, $id_equipBas, $id_satisFam, $id_evalFam, $_SESSION['consultorio']));
        $respuesta = array(
            'respuesta' => 'Correcto',
            'datos' => array(
                'id_editado' => $id_nuc
